<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="RiskDownThresh" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.risk_down_thresh }}</span>
        </template>
      </el-table-column>
      <el-table-column label="RiskUpThresh" width="150px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.risk_up_thresh }}</span>
        </template>
      </el-table-column>
      <el-table-column label="ActualRisk" width="100px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.actual_risk }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="150px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="RiskDownThresh" prop="risk_down_thresh">
          <el-input v-model="temp.risk_down_thresh" />
        </el-form-item>
        <el-form-item label="RiskUpThresh" prop="risk_up_thresh">
          <el-input v-model="temp.risk_up_thresh" />
        </el-form-item>
      </el-form>
...
<script>
import { fetchGlobal, updateGlobal } from '@/api/global'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      fetchGlobal(this.listQuery).then(response => {
